...
async function main() {
    ...
    const compiledSierra = json.parse(fs.readFileSync( "./target/dev/min_nft_market_MockERC721.sierra.json").toString( "ascii"));
    const compiledCasm = json.parse(fs.readFileSync( "./target/dev/min_nft_market_MockERC721.casm.json").toString( "ascii"));
